...
const ERP_CLIENT_VERSION = 'V30.58_SAFE_PRINT_FIX_FROM_V57'
...
function saveLocalNhapLieu(session, type, title, payload) {
  ...
  const localPayload = { ...payload, loaiBaoCao: loai, items: savedItems, localOnly: true, localDraftAt: payload.localDraftAt || nowLocal, cachedAt: nowLocal, savedAt: new Date().toISOString() }
  ...
  old.bundles[loai][title] = {
    ...
    localOnly: true, localDraftAt: payload.localDraftAt || nowLocal, cachedAt: nowLocal, savedAt: new Date().toISOString()
  }
...
// Lý do lỗi cũ: bỏ chọn 1 người rồi lưu, local còn 4 nhưng API cũ vẫn trả 5 trong 5-20s,
// màn chi tiết/modal nạp remote và hiện lại 5.
const LOCAL_DETAIL_LOCK_MS = 10 * 60 * 1000
function bundleTouchedAt(bundle) {
  if (!bundle) return 0
  const n = Number(bundle.localDraftAt || bundle.cachedAt || 0)
  if (n) return n
  const t = Date.parse(bundle.savedAt || bundle.updatedAt || '')
...
function bundleSignature(bundle) {
  ...
  const rows = items
    .map(normalizeRow)
    .filter(x => x.selected !== false && (x.maNv || x.tenNv))
    .map(x => [
      cacheTextKey(x.maNv || x.tenNv || ''),
      cacheTextKey(x.trangThai || ''),
      cacheTextKey(x.boPhanChuyenDen || x.toChuyenDen || ''),
    ].join(':'))
    .sort()
  return [
    rows.join('|'),
    cacheTextKey(bundle?.batDau || ''),
    cacheTextKey(bundle?.ketThuc || ''),
    String(bundle?.soGio || ''),
  ].join('@@')
}
function isFreshLocalBundle(bundle) {
  if (!bundle) return false
  const t = bundleTouchedAt(bundle)
  return !!(bundle.localOnly || bundle.localDraftAt || bundle.cachedAt) && t && (Date.now() - t < LOCAL_DETAIL_LOCK_MS)
}
function shouldKeepFreshLocalBundle(localBundle, remoteBundle) {
  if (!isFreshLocalBundle(localBundle)) return false
  if (!remoteBundle || !Array.isArray(remoteBundle.items)) return true
  return bundleSignature(localBundle) !== bundleSignature(remoteBundle)
}
...
function CompanyDeptCard({ row, index, moduleId, onOpen }) {
  const tong = deptNumber(row.tongCongNhan)
  const coMat = deptNumber(row.coMat)
  const vangSang = deptNumber(row.vangBuoiSang)
  ...
  return <button className="dept-card" onClick={onOpen}>
    ...
    <div className="dept-card-body">
      <div className="dept-card-top"><div><strong>{row.boPhan || 'Bộ phận'}</strong><span>Tổ trưởng: {row.toTruong || '-'}</span></div><em>›</em></div>
      <div className="dept-stats">{stats.map(([icon, label, value, tone]) => <DeptStat key={label} icon={icon} label={label} value={value} tone={tone} />)}</div>
    </div>
  </button>
}
...
function reportLoaiForModule(module) {
  if (!module) return ''
  if (module.id === 'tonghop' || module.id === 'vang') return 'Vắng mặt'
  if (module.id === 'tangca') return 'Tăng ca'
  if (module.id === 'ngayle') return 'Làm ngày lễ'
  if (module.id === 'biendong') return 'Chuyển bộ phận'
  return module.label || module.loai || ''
}
function rowValueAny(row, keys, fallback = '') {
  for (const key of keys) {
    if (row && row[key] !== undefined && row[key] !== null && row[key] !== '') return row[key]
  }
  return fallback
}
function normalizeDetailReportRow(row, module, title, deptName) {
  const maNv = rowValueAny(row, ['maNv','maNV','ma_nv','MA_NV','Mã NV','Mã nhân viên','ma','MANV'])
  const tenNv = rowValueAny(row, ['tenNv','hoTen','ten','HO_TEN','Họ và tên','Tên nhân viên','ho_va_ten','TEN_NV'])
  const boPhan = rowValueAny(row, ['boPhan','boPhanGoc','BO_PHAN','Bộ phận','Tổ','to','toSanXuat'], deptName)
  const soGio = rowValueAny(row, ['soGio','gio','SO_GIO','Giờ tăng ca','Giờ làm lễ','gioTangCa','gioLamLe'], '')
  const trangThai = rowValueAny(row, ['trangThai','lyDo','LY_DO','Lý do','noiDung','Nội dung','chiTiet','CHI_TIET'], title)
  const batDau = rowValueAny(row, ['batDau','BAT_DAU','Bắt đầu'], '')
  const ketThuc = rowValueAny(row, ['ketThuc','KET_THUC','Kết thúc'], '')
  return { ...row, maNv, tenNv, boPhan, boPhanGoc: boPhan, trangThai, soGio, batDau, ketThuc, selected: true }
}
function detailTitleMatch(row, title, module) {
  const raw = [
    rowValueAny(row, ['chiTiet','CHI_TIET','Chi tiết'], ''),
    rowValueAny(row, ['loaiTangCa','LOAI_TANG_CA','Loại tăng ca'], ''),
    rowValueAny(row, ['ca','Ca'], ''),
    rowValueAny(row, ['trangThai','TRANG_THAI','Trạng thái'], ''),
    rowValueAny(row, ['loai','LOAI','Loại'], ''),
  ].join(' ')
  const a = stripVietnamese(raw)
  const b = stripVietnamese(title)
  if (!a) return true
  if (a.includes(b) || b.includes(a)) return true
  if (module?.id === 'tangca') {
    if (b.includes('sang') && a.includes('sang')) return true
    if (b.includes('trua') && a.includes('trua')) return true
    if (b.includes('chieu') && a.includes('chieu')) return true
    if (b.includes('dot xuat') && (a.includes('dot xuat') || a.includes('khac'))) return true
  }
  if (module?.id === 'tonghop' || module?.id === 'vang') {
    if (b.includes('sang') && a.includes('sang')) return true
    if (b.includes('chieu') && a.includes('chieu')) return true
    if (b.includes('ca ngay') && (a.includes('ca ngay') || a.includes('ngay'))) return true
  }
  return false
}
function rowsFromReportResponse(res) {
  if (Array.isArray(res)) return res
  if (Array.isArray(res?.rows)) return res.rows
  if (Array.isArray(res?.items)) return res.items
  if (Array.isArray(res?.data)) return res.data
  if (Array.isArray(res?.chiTiet)) return res.chiTiet
  return []
}
async function fetchCompanyDetailReportBundle(module, deptName, title) {
  const loaiBaoCao = reportLoaiForModule(module)
  const params = {
    loaiBaoCao,
    boPhan: deptName,
    tuNgay: today(),
    denNgay: today(),
    chiTiet: title,
    loaiTangCa: module?.id === 'tangca' ? title : 'Tất cả',
  }
  const res = await api('getBaoCaoChiTietXemTruoc', [params])
  const rows = rowsFromReportResponse(res)
    .filter(r => detailTitleMatch(r, title, module))
    .map(r => normalizeDetailReportRow(r, module, title, deptName))
    .filter(r => r.maNv || r.tenNv)
    ...
    module.titles.forEach(title => {
      api('getNhapLieuBundleV309', [today(), dept.boPhan, module.loai, title])
        .then(async remote => {
          ...
          // V30.52: không cho API cũ ghi đè local vừa lưu; nếu bundle cũ trống thì lấy trực tiếp từ BAO_CAO_CHI_TIET.
          if (shouldKeepFreshLocalBundle(currentLocal, remote)) return
          let nextRemote = remoteItems.length ? remote : null
          if (!nextRemote && module.id !== 'tonghop') {
            nextRemote = await fetchCompanyDetailReportBundle(module, dept.boPhan, title).catch(() => null)
          }
          if (!nextRemote && (module.id === 'tonghop' || module.id === 'vang')) {
            nextRemote = await fetchCompanyDetailReportBundle(module, dept.boPhan, title).catch(() => null)
          }
          const nextItems = Array.isArray(nextRemote?.items) ? nextRemote.items : []
          if (!nextItems.length) return
          writeJson(detailBundleKey(today(), dept.boPhan, module.loai, title), nextRemote)
          setBundles(prev => ({ ...prev, [title]: nextRemote }))
          setMsg('')
        })
        .catch(() => {})
    })
...
function CompanyScreen({ session }) {
  ...
  const [activeModule, setActiveModule] = useState('tonghop')
...
function PrintOvertimeScreen({ session, departments }) {
  ...
  function rowLoaiBaoCao(r) {
    return String(cellValue(r, ['loaiBaoCao','LOAI_BAO_CAO','LOAI BAO CAO','loai_bao_cao','loai','LOAI'], '') || '').trim()
  }
  function rowChiTiet(r) {
    return String(cellValue(r, ['chiTiet','CHI_TIET','CHI TIET','chi_tiet','loaiTangCa'], '') || '').trim()
  }
  function filterRowsForCurrentReport(inputRows) {
    const list = normalizePreviewRows(inputRows)
    return list.filter(row => {
      const loai = rowLoaiBaoCao(row)
      const ct = rowChiTiet(row)
      if (loaiBaoCao === 'Tăng ca') {
        const okLoai = sameText(loai, 'Tăng ca') || ct.startsWith('Tăng ca')
        const okChiTiet = loaiTangCa === 'Tất cả' || sameText(ct, loaiTangCa)
        return okLoai && okChiTiet
      }
      if (loaiBaoCao === 'Làm ngày lễ') {
        return sameText(loai, 'Làm ngày lễ') || sameText(ct, 'Đăng ký làm ngày lễ')
      }
      if (loaiBaoCao === 'Vắng mặt') {
        return sameText(loai, 'Báo cáo vắng') || ct.startsWith('Vắng')
      }
      if (loaiBaoCao === 'Chuyển bộ phận') {
        return sameText(loai, 'Biến động nhân sự') && sameText(ct, 'Điều động sang tổ khác')
      }
      if (loaiBaoCao === 'Biến động') {
        return sameText(loai, 'Biến động nhân sự')
      }
      return true
    })
  }
  function dedupeOvertimeRows(inputRows) {
    ...
    ;(inputRows || []).forEach(r => {
      const key = [
        cellValue(r, ['maNv','ma','MA_NV']),
        cellValue(r, ['ngay','NGAY']),
        cellValue(r, ['chiTiet','loaiTangCa','CHI_TIET']),
        cellValue(r, ['batDau','BAT_DAU']),
        cellValue(r, ['ketThuc','KET_THUC'])
  ...
  async function fetchRowsForCurrentReport() {
    const params = reportParams()
    const tries = [
      // Kiểu mới: truyền object.
      () => api('getBaoCaoChiTietXemTruoc', [params]),
      // Kiểu cũ thường gặp trong Apps Script: truyền từng tham số.
      () => api('getBaoCaoChiTietXemTruoc', [params.loaiBaoCao, params.boPhan, params.tuNgay, params.denNgay, params.chiTiet, params.loaiTangCa]),
      // Dự phòng: yêu cầu đọc toàn bộ chi tiết rồi client tự lọc.
      () => api('getBaoCaoChiTietXemTruoc', [{ ...params, loaiBaoCao: 'Tất cả', chiTiet: 'Tất cả' }]),
      () => api('getBaoCaoChiTietXemTruoc', ['Tất cả', params.boPhan, params.tuNgay, params.denNgay, 'Tất cả', params.loaiTangCa]),
    ]
    let lastRows = []
    for (const fn of tries) {
      try {
        const r = await fn()
        const raw = normalizePreviewRows(r)
        const filtered = filterRowsForCurrentReport(raw)
        if (filtered.length) return filtered
        if (raw.length) lastRows = raw
      } catch (e) {}
    }
    // Chỉ riêng Tăng ca mới dùng API tăng ca cũ.
    if (loaiBaoCao === 'Tăng ca') {
      const r = await api('getDanhSachTangCaXemTruoc', ['THEO_KHOANG_NGAY', bp, fromInputDate(tuNgay), fromInputDate(denNgay), '', loaiTangCa])
      return filterRowsForCurrentReport(normalizePreviewRows(r))
    }
    return filterRowsForCurrentReport(lastRows)
  }
  async function load() {
    ...
    try {
      let data = await fetchRowsForCurrentReport()
      ...
      setRows([])
      setMsg(`❌ Không xem trước được ${reportTitle()}: ${e.message || 'Lỗi API.'}`)
    }
  ...
  async function exportExcel() {
    ...
    try {
      let data = rows.length ? filterRowsForCurrentReport(rows) : await fetchRowsForCurrentReport()
      if (loaiBaoCao === 'Tăng ca') data = dedupeOvertimeRows(data)
      setRows(data)
      writeJson(cacheKey, { rows: data, cachedAt: Date.now(), loaiBaoCao, loaiTangCa: showLoaiTangCa ? loaiTangCa : '' })
  ...
  async function shareZalo() {
    ...
    const shareText = `File ${reportTitle()}: ${fileUrl}`
    if (navigator.share) {
      try {
        await navigator.share({ title: `File ${reportTitle()}`, text: shareText, url: fileUrl })
