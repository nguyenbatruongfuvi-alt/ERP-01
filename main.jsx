...
const LOCAL_SAVE_PREFIX = 'erp_v30_local_first_v49'
const PRELOAD_PREFIX = 'erp_v30_today_preload_v49'
const LAST_DEPT_KEY = 'erp_v30_last_department'
const BOOT_KEY = 'erp_v30_boot_init_v49'
...
// =========================
// V30.49 LOCAL DETAIL INSTANT FIX
// Lưu cache chi tiết ngay sau khi bấm Lưu để mở lại thấy dữ liệu liền,
// không chờ Google Sheet/Apps Script trả dữ liệu mới.
const LOCAL_DETAIL_PREFIX = 'erp_v30_detail_cache_v49'
function normKeyPart(value) {
  return stripVietnamese(String(value || 'tat_ca')).replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || 'tat_ca'
}
...
function detailCacheKey(loaiBaoCao, boPhan, loaiTangCa, tuNgay, denNgay) {
  return `${LOCAL_DETAIL_PREFIX}_${normKeyPart(loaiBaoCao)}_${normKeyPart(boPhan)}_${normKeyPart(loaiTangCa || 'tat_ca')}_${normKeyPart(tuNgay)}_${normKeyPart(denNgay)}`
}
function detailKindFromPayload(type, title, payload) {
  if (payload?.loaiBaoCao === 'Làm ngày lễ' || type === 'Làm ngày lễ') return { loaiBaoCao: 'Làm ngày lễ', loaiTangCa: 'Tất cả' }
  if (payload?.loaiBaoCao === 'Biến động nhân sự' || type === 'Biến động') {
    if (title === 'Điều động sang tổ khác') return { loaiBaoCao: 'Chuyển bộ phận', loaiTangCa: 'Tất cả' }
    return { loaiBaoCao: 'Biến động', loaiTangCa: 'Tất cả' }
  }
  if (payload?.loaiBaoCao === 'Báo cáo vắng' || type === 'Vắng mặt') return { loaiBaoCao: 'Vắng mặt', loaiTangCa: 'Tất cả' }
  return { loaiBaoCao: 'Tăng ca', loaiTangCa: title || 'Tất cả' }
}
function localDetailRowFromItem(item, payload, type, title) {
  return {
    ...
    ngay: payload.ngay,
    NGAY: payload.ngay,
    maNv: item.maNv || item.ma || '',
    MA_NV: item.maNv || item.ma || '',
    tenNv: item.tenNv || item.ten || '',
    TEN_NV: item.tenNv || item.ten || '',
    boPhan: payload.boPhan,
    boPhanGoc: item.boPhanGoc || item.boPhan || payload.boPhan,
    BO_PHAN_GOC: item.boPhanGoc || item.boPhan || payload.boPhan,
    loaiBaoCao: payload.loaiBaoCao,
    LOAI_BAO_CAO: payload.loaiBaoCao,
    chiTiet: title,
    CHI_TIET: title,
    batDau: payload.batDau || '',
    ketThuc: payload.ketThuc || '',
    soGio: payload.soGio || '',
    trangThai: item.trangThai || 'Đã chọn',
    TRANG_THAI: item.trangThai || 'Đã chọn',
    toChuyenDen: item.toChuyenDen || payload.toChuyenDen || payload.boPhanChuyenDen || '',
    TO_CHUYEN_DEN: item.toChuyenDen || payload.toChuyenDen || payload.boPhanChuyenDen || '',
    ghiChu: payload.ghiChu || item.ghiChu || ''
  }
}
function saveLocalDetailRows(session, type, title, payload) {
  const kind = detailKindFromPayload(type, title, payload)
  const ngay = payload.ngay || today()
  const rows = (payload.items || []).map(item => localDetailRowFromItem(item, { ...payload, ngay }, type, title))
  const keys = [
    detailCacheKey(kind.loaiBaoCao, session.boPhan, kind.loaiTangCa, ngay, ngay),
    detailCacheKey(kind.loaiBaoCao, 'Tất cả', kind.loaiTangCa, ngay, ngay),
    detailCacheKey(kind.loaiBaoCao, session.boPhan, 'Tất cả', ngay, ngay),
    detailCacheKey(kind.loaiBaoCao, 'Tất cả', 'Tất cả', ngay, ngay),
  ]
  keys.forEach(key => {
    const old = readJson(key, { rows: [] })
    const oldRows = Array.isArray(old?.rows) ? old.rows : []
    const filtered = oldRows.filter(r => !(r.requestId && r.requestId === payload.requestId))
    writeJson(key, { rows: [...rows, ...filtered], cachedAt: Date.now(), localInstant: true })
  })
}
function readLocalDetailRows(loaiBaoCao, boPhan, loaiTangCa, tuNgay, denNgay) {
  const key = detailCacheKey(loaiBaoCao, boPhan, loaiTangCa, tuNgay, denNgay)
  const direct = readJson(key, null)
  if (direct?.rows) return direct.rows
  const start = inputDateToMs(tuNgay), end = inputDateToMs(denNgay)
  if (!start || !end) return []
  const rows = []
  for (let t = start; t <= end; t += 86400000) {
    const d = new Date(t)
    const dd = `${pad2(d.getDate())}/${pad2(d.getMonth() + 1)}/${d.getFullYear()}`
    const one = readJson(detailCacheKey(loaiBaoCao, boPhan, loaiTangCa, dd, dd), null)
    if (one?.rows) rows.push(...one.rows)
  }
  return rows
}
function mergeUniqueRows(localRows, remoteRows) {
  const map = new Map()
  ;[...(localRows || []), ...(remoteRows || [])].forEach((r, i) => {
    const key = [r.requestId || '', r.maNv || r.MA_NV || '', r.ngay || r.NGAY || '', r.chiTiet || r.CHI_TIET || '', r.trangThai || r.TRANG_THAI || '', i].join('|')
...
function saveLocalNhapLieu(session, type, title, payload) {
  ...
  saveLocalDetailRows(session, type, title, payload)
...
function PrintOvertimeScreen({ session, departments }) {
  ...
  const localRowsForFilter = () => readLocalDetailRows(loaiBaoCao, bp, showLoaiTangCa ? loaiTangCa : 'Tất cả', fromInputDate(tuNgay), fromInputDate(denNgay))

  useEffect(() => {
    const localRows = localRowsForFilter()
    const cached = readJson(cacheKey, null)
    const merged = mergeUniqueRows(localRows, cached?.rows || [])
    if (merged.length) {
      setRows(merged)
      setMsg(localRows.length ? 'Đang hiển thị dữ liệu đã lưu tức thì trên máy.' : 'Đang hiển thị dữ liệu đã lưu trên máy.')
    } else {
      setRows([])
      setMsg('')
    }
  ...
  async function load() {
    ...
    try {
      ...
      const remoteRows = normalizePreviewRows(r)
      const localRows = localRowsForFilter()
      const data = mergeUniqueRows(localRows, remoteRows)
      setRows(data)
